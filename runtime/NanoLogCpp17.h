 ...
 * about the n-th parameter that would be used in a corresponding NANO_LOG()
 * invocation.
 *
 * \tparam N
 ...
 * \param paramNum
 *      n-th parameter to return type information for (starts from zero)
 * \return
             ...
             int paramNum=0)
{
    ...
    // "if (...) {... continue; }" without an else in constexpr functions.
    // Thus we avoid them below. I reported this bug to the developers here
    ...
    while (pos < N - 1) {
        ...
        // about precision and type so we ignore everything.
        if (fmt[pos] == '%') {
            ++pos;
 ...
 *
 * \template NParams
 *      The exact number of format parameters that are in the format string
 *      This must be passed in since the return type depends on it
 * \template N
...
store_arguments(const std::array<ParamType, N>& paramTypes,
                ...
                Ts... rest)
{
    // Peal off one argument to store, and then recursively process rest
...
#pragma GCC diagnostic ignored "-Wfloat-conversion"
    if (fmtType == ParamType::DYNAMIC_PRECISION)
        previousPrecision = static_cast<uint64_t>(arg);
#pragma GCC diagnostic pop
...
#pragma GCC diagnostic pop
        ...
        bzero(*out + stringBytes, characterWidth);
        *out += characterWidth;
        return;
    }

    // The argument is a basic type, so store it if we're not processing strings
    if (stringsOnly) {
        *in += sizeof(T);
        return;
    }
 ...
 * \param filename
 *      Static filename associated with the log invocation
 * \param linenum
 *      Static line number associated with the log invocation
 * \param severity
    ...
    Ts... args)
{
    ...
    if (logId == UNASSIGNED_LOGID) {
        ...
        RuntimeLogger::registerInvocationSite(logId, info);
    }
